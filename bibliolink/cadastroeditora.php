...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nome)) {
        $mensagem = "Por favor, preencha o nome da editora.";
    } else {
        // Prepara a consulta para inserir a editora no banco de dados
        $stmt = $conn->prepare("INSERT INTO editoras (Nome) VALUES (?)");
        $stmt->bind_param("s", $nome);

        if ($stmt->execute()) {
            // Exibe a mensagem de sucesso e mantém a página
            $mensagem = "Editora cadastrada com sucesso! O ID gerado automaticamente é: " . $stmt->insert_id;
        } else {
            $mensagem = "Erro ao cadastrar a editora.";
        }
    }
}
?>
...
<head>
    ...
    <title>Cadastro de Editora - Bibliolink</title>
    <style>
        body {
...
<body>
    <div class="cadastro-container">
        ...
        <form action="cadastroeditora.php" method="POST">
            ...
            <input type="text" id="nome" name="nome" placeholder="Digite o nome da editora" required>
